...
int countSSE (char* string, int size) {
	...
	while (size > 16) {
		...
		int cur = _mm_popcnt_u64((unsigned long long) (reinterpret_cast<unsigned long long *> (&diff)[0])) +
                  _mm_popcnt_u64((unsigned long long) (reinterpret_cast<unsigned long long *> (&diff)[1]));
        cnt += (cur / 8);
        i += 16;
        size -= 16;
	}
